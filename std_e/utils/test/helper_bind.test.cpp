...
{
  int         n_cell            = 100;
  std::size_t val_can_be_changed = 1;

  SUBCASE(" Without std::ref, std::bind takes a value") {
    auto f_t1 = std_e::make_bind(test_helper_bind_1, n_cell, val_can_be_changed);
    f_t1();
    CHECK(val_can_be_changed == 1);
  }

  SUBCASE(" With std::ref std::bind takes the proper address") {
    auto f_t1 = std_e::make_bind(test_helper_bind_1, n_cell, std::ref(val_can_be_changed));
    f_t1();
    CHECK(val_can_be_changed == 2);
  }
...
TEST_CASE("[helper_bind] make_bind with one tuple arg")
{
  int         n_cell            = 100;
  std::size_t val_can_be_changed = 1;

  SUBCASE(" Without std::ref std::bind takes a value") {
    auto t1   = std::make_tuple(n_cell, val_can_be_changed);
  ...
  SUBCASE(" With std::ref std::bind takes the proper address") {
    auto t1   = std::make_tuple(std::ref(n_cell), std::ref(val_can_be_changed));
    auto f_t1 = std_e::make_bind(test_helper_bind_1, t1);
