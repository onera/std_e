...
MPI_TEST_CASE("all_gather_vect",3) {
  ...
  std_e::all_gather(sz, recvcounts.data(), test_comm);
  int total_sz = std::accumulate(recvcounts.begin(),recvcounts.end(),0); 

  std::vector<int> rdata(total_sz);

  std_e::all_gather(value, rdata.data(), recvcounts, test_comm);
