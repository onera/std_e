...
all_to_all_v(
  const T* sbuf, const int* sstrides, const int* sindices,
        T* rbuf, const int* rstrides, const int* rindices,
  MPI_Comm comm,
  F algo_family = {}
) -> void
{
...
template<class Range, class Int_range, class F = dense_algo_family> auto
all_to_all_v_from_indices_tuple_aux(
    const Range& sbuf, const std::vector<int>& sstrides, const Int_range& sindices,
                       const std::vector<int>& rstrides, const interval_vector<int>& rindices,
    MPI_Comm comm, F algo_family = {}
) {
  using T = typename Range::value_type;
  std::vector<T> rbuf(rindices.length());
  all_to_all_v(
...
template<class Int_range, class F, class... Ranges> auto
all_to_all_v_from_indices_aux(const Int_range& sindices, MPI_Comm comm, F algo_family, const Ranges&... sbufs) {
  std::vector<int> sstrides = interval_lengths(sindices);
  std::vector<int> rstrides = all_to_all(sstrides,comm,algo_family.all_to_all);
  interval_vector<int> rindices = indices_from_strides(rstrides);
  ...
  return
    std::make_pair(
      std::make_tuple(
        all_to_all_v_from_indices_tuple_aux(sbufs,sstrides,sindices, rstrides,rindices, comm, algo_family)...
      ),
      std::move(rindices)
    );
}
