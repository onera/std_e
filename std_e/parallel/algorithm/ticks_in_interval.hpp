...
to_string_loc = [](const interval_to_partition<I>& x) {
  return
      "["+std::to_string(x.inf)+','+std::to_string(x.sup)+"):"
    + std::to_string(x.n_ticks)+':'+std::to_string(x.position);
};
template<class I> auto
to_string(const interval_to_partition<I>& x) {
  return
      "["+std::to_string(x.inf_tot)+','+std::to_string(x.sup_tot)+"):"
    + std::to_string(x.n_ticks);
}

template<class Rng, class I> auto
search_intervals3(const Rng& partition_indices, I max_interval_tick_shift, MPI_Comm comm) {
  ...
  I sz_tot = partition_indices_tot.back();
  ...
  std::vector<interval_to_partition<I>> sub_ins(n_sub_intervals);
  for (int i=0; i<n_sub_intervals; ++i) {
    I inf = partition_indices[interval_start[i]];
    I sup = partition_indices[interval_start[i] + 1];
    I inf_tot = partition_indices_tot[interval_start[i]];
    I sup_tot = partition_indices_tot[interval_start[i] + 1];
