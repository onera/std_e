...
template<
  class C, int N,
  template<class,auto> class val_type = std::array,
...
class block_range {
  public:
    ...
    using iterator = block_iterator<T,N,value_type,reference>;
    using const_iterator = block_iterator<const T,N,value_type,reference>;
