...
is_interval(const Rng& off) -> bool {
  return (off.size() >= 1) && std::is_sorted(off.begin(),off.end());
}


template<class Number, class Rng> auto
interval_index(Number x, const Rng& off) -> ptrdiff_t {
  auto it = std::upper_bound(off.begin(),off.end(),x);
  return it-begin(off)-1;
}


template<class Rng, class Out_rng> auto
offset_to_size(const Rng& off, Out_rng& bsz) -> void {
  STD_E_ASSERT(bsz.size() >= off.size()-1);
  std_e::exclusive_adjacent_difference(off.begin(),off.end(),bsz.begin());
}
template<class Rng, class Out_rng> auto
size_to_offset(const Rng& bsz, Out_rng& off) -> void {
...
offset_to_size(const Rng& off) -> std_e::dynarray<T> {
  dynarray<T> bsz(off.size()-1);
  offset_to_size(off, bsz);
  return bsz;
}
template<class Rng, class T = typename Rng::value_type> auto
size_to_offset(const Rng& bsz) -> std_e::dynarray<T> {
  dynarray<T> off(bsz.size()+1);
  size_to_offset(bsz, off);
  return off;
}


template<class Rng> auto
inplace_size_to_offset(Rng& bsz) -> Rng& {
