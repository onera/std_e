...
> constexpr auto
pivot_partition_indices(RA_rng& x, const RA_pivot_rng& pivots, Proj proj = {}, Comp comp = {}, Return_container&& partition_is = {}) -> Return_container {
  int n = x.size();
  int k = pivots.size();
  partition_is.resize(1+k+1); // also add begin and end for convenience
  partition_is[0] = 0;
  pivot_partition_indices(begin(x),end(x), begin(pivots),end(pivots), begin(partition_is)+1, proj,comp);
  partition_is.back() = n;
  return partition_is;
}
