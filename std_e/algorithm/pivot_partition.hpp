...
template<class RA_it, class RA_it_piv, class RA_it_out, class Proj, class Comp> constexpr auto
pivot_partition_i_aux(RA_it first, RA_it last, RA_it_piv piv_first, RA_it_piv piv_last, RA_it_out out_first, Proj proj, Comp comp, RA_it start) -> void {
  if (piv_first==piv_last) return;
  auto k = piv_last-piv_first;
  auto pv_mid = piv_first+k/2;
  auto pi_mid = out_first+k/2;
  auto pp_mid = std::partition(first,last,[&mid=*pv_mid,proj,comp](const auto& x){ return comp(proj(x),mid); });
  *pi_mid = pp_mid - start;
  if (k>1) {
    pivot_partition_i_aux(first ,pp_mid , piv_first,pv_mid  , out_first, proj,comp, start);
    pivot_partition_i_aux(pp_mid,last   , pv_mid+1,piv_last , pi_mid+1 , proj,comp, start);
  }
}

template<class RA_it, class RA_it_piv, class RA_it_out, class Proj = identity_closure, class Comp = std::less<>> constexpr auto
pivot_partition_indices(RA_it first, RA_it last, RA_it_piv piv_first, RA_it_piv piv_last, RA_it_out out_first, Proj proj = {}, Comp comp = {}) -> void {
  return pivot_partition_i_aux(first,last,piv_first,piv_last,out_first,proj,comp,first);
}
// pivot_partition_indices }
