...
TEST_CASE("distribution_weighted_by_blocks") {
  SUBCASE("uniform") {
    ...
    SUBCASE("numbers_of_elt_in_interval - less blocs than slots") {
      ...
      CHECK( v          == std::vector{0,50,100,150,200,250,300,350,400,450,500} );
      CHECK( v_weighted == std::vector{0,50,100,150,200,250,300,350,400,450,500} );
  ...
  SUBCASE("simple") {
    ...
    auto [v,v_weighted] = distribution_weighted_by_blocks(n_slot,sizes,weights);
    CHECK( v == std::vector{0,8,18} );
    CHECK( v_weighted == std::vector{0,32,69} );
  }
}
