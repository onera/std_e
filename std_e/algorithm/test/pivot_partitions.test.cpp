...
#include "std_e/algorithm/multi_partition.hpp"
...
// TODO or rename partitions/multi_partition, partitions_i/multi_partition_i, partition_pts, partition_points_i


// [Sphinx Doc] pivot partitions {
TEST_CASE("pivot partitions") {
  vector<int> v = {110,200,2,6,-3,800,10001,999,8,50,0};

  vector<int> pivots = {0,10,100,120,1000};

  using it_type = typename vector<int>::iterator;
  vector<it_type> partition_pts(pivots.size());
  std_e::pivot_partitions(begin(v),end(v), begin(pivots),end(pivots), begin(partition_pts));
...
// [Sphinx Doc] pivot partitions }
  ...
  //  auto partition_pts = std_e::partition_sort(v,pivots);
  //  //                    0        10   100    120            1000
  //  CHECK( v == vector{-3, 8,2,6,0,  50,   110,   999,800,200,     10001} );
  //}
  //SUBCASE("indices vector-returning algorithm") {
  //  auto partition_indices = std_e::partition_sort_indices(v,pivots);
  //  //                                    0        10   100    120            1000
  //  CHECK(                 v ==          vector{-3, 8,2,6,0,  50,   110,   999,800,200,     10001} );
...
//  vector<int> pivots = {0,10,100,120,1000};
//
//  auto partition_indices = std_e::partition_indices(v,pivots);
