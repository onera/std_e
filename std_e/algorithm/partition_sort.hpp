...
// Same with indices return instead of pointers
template<class Rand_it0, class Rand_it1, class Rand_it2, class Bin_pred> constexpr auto
partition_indices__impl(Rand_it0 first, Rand_it0 last, Rand_it1 pv_first, Rand_it1 pv_last, Rand_it2 pi_first, Bin_pred comp, Rand_it0 start) -> void {
  if (pv_first==pv_last) return;
  auto k = pv_last-pv_first;
  auto pv_mid = pv_first+k/2;
  auto pi_mid = pi_first+k/2;
  ...
  *pi_mid = pp_mid - start;
  if (k>1) {
    ...
    partition_indices__impl(pp_mid,last   , pv_mid  ,pv_last , pi_mid   , comp, start);
  }
}
...
template<class Rand_it0, class Rand_it1, class Rand_it2, class Bin_pred = std::less<>> constexpr auto
partition_indices(Rand_it0 first, Rand_it0 last, Rand_it1 pv_first, Rand_it1 pv_last, Rand_it2 pi_first, Bin_pred comp = {}) -> void {
  return partition_indices__impl(first,last,pv_first,pv_last,pi_first,comp,first);
}
...
partition_indices(const Rand_range0& rng, const Rand_range1& partition_values, Bin_pred comp = {}) -> std::vector<int> {
  int k = partition_values.size();
  std::vector<int> partition_is(k);
  partition_indices(begin(rng),end(rng),begin(partition_values),end(partition_values),begin(partition_is),comp);
  return partition_is;
}
...
indirect_partition_sort(const Rand_range0& rng, const Rand_range1& partition_values, Bin_pred comp = {}) {
  ...
  auto partition_is = std_e::partition_sort_indices(new_to_old,partition_values,indirect_comp);
