...
shape_restriction_start_index_1d(const Multi_array_shape& x, const Multi_index& right_indices) {
  using index_type = typename Multi_array_shape::index_type;
  ...
  auto sub_view_start_index = std_e::concatenate(std_e::multi_index<index_type,sub_rank>{0},right_indices);
