...
struct index_type__impl;

template<class T, template<class> class OR, template<class> class IR>
struct index_type__impl<range_of_ranges<T,OR,IR>> {
  using type = T;
};
template<class adjacency_list_type>
using value_type = typename index_type__impl<adjacency_list_type>::type;
/// value_type }
...
template<class adjacency_list_type>
using range_from_outer = typename range_from_outer__impl<adjacency_list_type>::type;
/// range_from_outer }
