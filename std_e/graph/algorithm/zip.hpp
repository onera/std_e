...
using graph_zip = zpair<G0,G1>;

template<class G0, class G1>
using graph_zip_view = zpair<G0&,G1&>;

template<class T>
struct is_graph_zip__impl {
  static constexpr bool value = false;
};
template<class G0, class G1>
struct is_graph_zip__impl<graph_zip<G0,G1>> { // will also match graph_zip_view
  static constexpr bool value = true;
};
template<class T>
constexpr bool is_graph_zip_view = is_graph_zip__impl<std::decay_t<T>>::value;
